...
import { generateDocumentPdf } from "@/app/_lib/serverFunctions/pdf";
import { getSessionAndUser } from "@/app/_lib/serverFunctions/auth";
import { prisma } from "@/app/_lib/constants/prisma";
import { NextRequest, NextResponse } from "next/server";
import { generateDocumentPdfHarbstone } from "@/app/_lib/serverFunctions/pdf-harbstone";
...
export async function GET(req: NextRequest, { params }: ParamsObject) {
  try {
    ...
    const isHarbstone = searchParams.get("isHarbstone") || "";
    ...
    const res =
      isHarbstone === "true"
        ? await generateDocumentPdfHarbstone(document)
        : await generateDocumentPdf(document);
    if (!res.ok) return NextResponse.json(getResult(false, 3), { status: 400 });
    const bytes = res.data;

    const filename = getDocumentFilename(document, "pdf");
